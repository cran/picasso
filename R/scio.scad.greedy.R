#----------------------------------------------------------------------------------#
# Package: picasso                                                                  #
# scio.scad.greedy(): Sparse Column Inverse Operator                               #
# Author: Xingguo Li                                                               #
...
scio.scad.greedy <- function(S, lambda, nlambda, gamma, d, maxdf, prec, max.ite, verbose){
  
  if(verbose==TRUE)
    cat("SCAD regularization via greedy actic set identification and coordinate descent\n")
  d_sq = d^2
  ...
  begt=Sys.time()
  str=.C("picasso_scio_scad_greedy", as.double(S), as.double(icov), 
         ...
         as.integer(row_idx),as.double(gamma), PACKAGE="picasso")
  runt1=Sys.time()-begt
