#----------------------------------------------------------------------------------#
# Package: picasso                                                                  #
# logit.map.stoc(): Stochastic actic set identification                              #
# Author: Xingguo Li                                                               #
...
logit.mcp.stoc <- function(Y, X, lambda, nlambda, gamma, n, d, max.ite, prec,verbose)
{
  if(verbose==TRUE)
    cat("MCP regularization via stochastic actic set identification and coordinate descent\n")
  beta = matrix(0,nrow=d,ncol=nlambda)
  beta.intcpt = rep(0,nlambda)
  size.act = rep(0,nlambda)
  ite.lamb = rep(0,nlambda)
  str=.C("picasso_logit_mcp_stoc", as.double(Y), as.double(X), 
