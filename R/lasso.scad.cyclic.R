#----------------------------------------------------------------------------------#
# Package: picasso                                                                  #
# lasso.scad.cyclic(): Cyclic actic set identification                             #
# Author: Xingguo Li                                                               #
...
lasso.scad.cyclic <- function(Y, X, lambda, nlambda, gamma, n, d, max.ite, prec,verbose)
{
  if(verbose==TRUE)
    cat("Lasso SCAD via Cyclic Coordinate Descent\n")
  S = colSums(X^2)
  beta = matrix(0,nrow=d,ncol=nlambda)
  beta.intcpt = rep(0,nlambda)
  size.act = rep(0,nlambda)
  ite.lamb = rep(0,nlambda)
  str=.C("picasso_lasso_scad_cyclic", as.double(Y), as.double(X), as.double(S), 
