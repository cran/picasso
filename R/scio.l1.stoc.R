#----------------------------------------------------------------------------------#
# Package: picasso                                                                  #
# scio.l1.stoc(): Sparse Column Inverse Operator                                   #
# Author: Xingguo Li                                                               #
...
scio.l1.stoc <- function(S, lambda, nlambda, d, maxdf, prec, max.ite, verbose){
  ...
  begt=Sys.time()
  str=.C("picasso_scio_l1_stoc", as.double(S), as.double(icov), 
         as.integer(d), as.integer(ite.lamb), as.double(lambda), 
         as.integer(nlambda), as.integer(max.ite), as.double(prec), 
         as.double(x), as.integer(col_cnz), as.integer(row_idx),PACKAGE="picasso")
  runt1=Sys.time()-begt
