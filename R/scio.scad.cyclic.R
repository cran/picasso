#----------------------------------------------------------------------------------#
# Package: picasso                                                                  #
# scio.scad.cyclic(): Sparse Column Inverse Operator                               #
# Author: Xingguo Li                                                               #
...
scio.scad.cyclic <- function(S, lambda, nlambda, gamma, d, maxdf, prec, max.ite, verbose){
  
  if(verbose==TRUE)
    cat("SCAD regularization via cyclic actic set identification and coordinate descent\n")
  d_sq = d^2
  ...
  begt=Sys.time()
  str=.C("picasso_scio_scad_cyclic", as.double(S), as.double(icov), 
