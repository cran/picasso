#----------------------------------------------------------------------------------#
# Package: picasso                                                                  #
# mvr.greedy(): Multivariate regression with greedy actic set identification       #
# Author: Xingguo Li                                                               #
...
mvr.greedy <- function(Y, X, lambda, nlambda, n, d, m, max.ite, prec, verbose)
{
  if(verbose==TRUE)
    cat("L12 regularization via greedy actic set identification and coordinate descent.\n")
  S = colSums(X^2)
  ...
  str=.C("picasso_mvr_greedy", as.double(Y), as.double(X), as.double(S), 
