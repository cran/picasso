#----------------------------------------------------------------------------------#
# Package: picasso                                                                  #
# lasso.mcp.prox(): Proximal gradient actic set identification                     #
# Author: Xingguo Li                                                               #
...
lasso.mcp.prox <- function(Y, X, lambda, nlambda, gamma, n, d, max.ite, prec,verbose)
{
  if(verbose==TRUE)
    cat("MCP via proximal gradient actic set identification and coordinate descent\n")
  Sigma = crossprod(X)
  ...
  str=.C("picasso_lasso_mcp_prox", as.double(Y), as.double(X), as.double(S), 
