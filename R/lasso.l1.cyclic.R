#----------------------------------------------------------------------------------#
# Package: picasso                                                                  #
# lasso.l1.cyclic(): Cyclic actic set identification                               #
# Author: Xingguo Li                                                               #
...
lasso.l1.cyclic <- function(Y, X, lambda, nlambda, n, d, max.ite, prec,verbose)
{
  if(verbose==TRUE)
    cat("L1 regularization via cyclic actic set identification and coordinate descent\n")
  S = colSums(X^2)
  beta = matrix(0,nrow=d,ncol=nlambda)
  beta.intcpt = rep(0,nlambda)
  size.act = rep(0,nlambda)
  ite.lamb = rep(0,nlambda)
  str=.C("picasso_lasso_l1_cyclic", as.double(Y), as.double(X), as.double(S), 
