#----------------------------------------------------------------------------------#
# Package: picasso                                                                  #
# glasso.cyclic(): Cyclic actic set identification                                 #
# Author: Xingguo Li                                                               #
...
glasso.cyclic <- function(Y, X, gr, gr.n, gr.size, lambda, nlambda, n, d, max.ite, prec, verbose)
{
  if(verbose==TRUE)
    cat("Group Lasso via Cyclic Coordinate Descent and Root Finding .\n")
  S = colSums(X^2)
  ...
  str=.C("picasso_glasso_cyclic", as.double(Y), as.double(X), as.double(S), 
