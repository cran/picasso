#----------------------------------------------------------------------------------#
# Package: picasso                                                                  #
# scio.mcp.prox(): Sparse Column Inverse Operator                                  #
# Author: Xingguo Li                                                               #
...
scio.mcp.prox <- function(S, lambda, nlambda, gamma, d, maxdf, prec, max.ite, verbose){
  
  if(verbose==TRUE)
    cat("MCP regularization via proximal gradient actic set identification and coordinate descent\n")
  ...
  begt=Sys.time()
  str=.C("picasso_scio_mcp_prox", as.double(S), as.double(icov), 
