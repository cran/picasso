#----------------------------------------------------------------------------------#
# Package: picasso                                                                  #
# glasso.greedy(): Greedy actic set identification                                 #
# Author: Xingguo Li                                                               #
...
glasso.greedy <- function(Y, X, gr, gr.n, gr.size, lambda, nlambda, n, d, max.ite, prec, verbose)
{
  if(verbose==TRUE)
    cat("Group Lasso via Greedy Coordinate Descent and Root Finding .\n")
  S = colSums(X^2)
  ...
  str=.C("picasso_glasso_greedy", as.double(Y), as.double(X), as.double(S), 
         ...
         as.integer(gr1),as.integer(gr.n),
