#----------------------------------------------------------------------------------#
# Package: picasso                                                                  #
# scio.l1.greedy(): Sparse Column Inverse Operator                                 #
# Author: Xingguo Li                                                               #
...
scio.l1.greedy <- function(S, lambda, nlambda, d, maxdf, prec, max.ite, verbose){
  
  if(verbose==TRUE)
    cat("Sparse column inverse operator via Greedy Coordinate Descent\n")
  d_sq = d^2
  ...
  begt=Sys.time()
  str=.C("picasso_scio_l1_greedy", as.double(S), as.double(icov), 
         ...
         as.integer(row_idx),PACKAGE="picasso")
  runt1=Sys.time()-begt
