#----------------------------------------------------------------------------------#
# Package: picasso                                                                  #
# scio.mcp.greedy(): Sparse Column Inverse Operator                                #
# Author: Xingguo Li                                                               #
...
scio.mcp.greedy <- function(S, lambda, nlambda, gamma, d, maxdf, prec, max.ite, verbose){
  
  if(verbose==TRUE)
    cat("MCP regularization via greedy actic set identification and coordinate descent\n")
  d_sq = d^2
  ...
  begt=Sys.time()
  str=.C("picasso_scio_mcp_greedy", as.double(S), as.double(icov), 
