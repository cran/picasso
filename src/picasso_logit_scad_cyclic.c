...
void picasso_logit_scad_cyclic(double *Y, double * X, double * beta, double * intcpt, int * nn, int * dd, int * ite1, int * size_act, double *lambda, int *nnlambda, double *ggamma, int *mmax_ite, double *pprec){
    ...
    for (i=0; i<nlambda; i++) {
        ...
        for(j=0; j<d; j++){
            ...
            // g = get_grad_logit(p, Y, X+j*n, n); // g = <p-Y, X>/n
            g = get_grad_logit_scad(p, Y, X+j*n, beta1[j], ilambda0, gamma, n); // g = <p-Y, X>/n + h_grad(scad)
            tmp = beta1[j] - g/w;
            if(fabs(tmp)>ilambda){
                comb_flag = 1;
                ...
                while (dif>prec && ite<max_ite) {
                    ...
                    for (m=0; m<size_a; m++) {
                        c_idx = set_act[m];
                        p_update(p,Xb,intcpt[i],n); // p[i] = 1/(1+exp(-intcpt-Xb[i]))
                        // g = get_grad_logit(p, Y, X+c_idx*n, n); // g = <p-Y, X>/n
