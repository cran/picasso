...
void picasso_logit_gr_cyclic_orth(double *Y, double * X, double * beta, double * intcpt, int * nn, int * dd, int * ite_lamb, int * ite_cyc, int * gr_size_act, double *obj, double *runt, int * gr, int * gr_nn, int * gr_size, double *lambda, int *nnlambda, double * ggamma, int *mmax_ite, double *pprec, int *fflag, int *mmax_act_gr_in, double *ttrunc){
    
    int i, j, k, m, n, d, gr_n, max_ite1, max_ite2, nlambda, gr_size_a, gr_size_a1, match, ite1, ite2, c_idx, gr_idx, flag, max_act_gr_in, act_gr_in, idx;
    double gamma, w, wn, prec1, prec2, ilambda, ilambda0, tmp, dif1, dif2, neg1, pos1, ilambda1, trunc;
    clock_t start, stop;
        ...
        while (dif1>prec1 && ite1<max_ite1) {
            ...
            for(j=0; j<gr_n; j++){
                ...
                if(match == 0){ // if j in set_act
                    if(flag==1){
                        get_grad_logit_gr_l1(grad, p_Y, X, gr[j], gr_size[j], n); // g[gr] = <p-Y, X[,gr]>/n
                    }
                    if(flag==2){
                        get_grad_logit_gr_mcp(grad, p_Y, X, beta1, gr[j], gr_size[j], ilambda0, gamma, n); // g[gr] = <p-Y, X[,gr]>/n + h_grad(mcp)
                    }
                    if(flag==3){
                        get_grad_logit_gr_scad(grad, p_Y, X, beta1, gr[j], gr_size[j], ilambda0, gamma, n); // g[gr] = <p-Y, X[,gr]>/n + h_grad(scad)
                    }
                    logit_gr_vec_dif(beta_tild, beta1, grad, w, gr[j], gr_size[j]); //beta_tild[gr] = beta1[gr] - g/w
                    tmp = norm2_gr_vec(beta_tild, gr[j], gr_size[j]);
                    if(tmp>ilambda1){
                        gr_act[gr_size_a] = j;
                        gr_size_a++;
                        act_gr_in++;
            ...
            while (dif2>prec2 && ite2<max_ite2) {
                ...
                for (m=0; m<gr_size_a; m++) {
                    ...
                    dif_vec(p_Y, p, Y, n); // p_Y = p - Y
                    if(flag==1){
                        get_grad_logit_gr_l1(grad, p_Y, X, gr[c_idx], gr_size[c_idx], n); // g[gr] = X[gr]^T (p-Y)/n
                    }
                    if(flag==2){
