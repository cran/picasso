...
// X n by d
// beta d by p
void picasso_mvr_prox_orth(double *Y, double * X, double * S, double * beta, double * intcpt, int * nn, int * dd, int * pp, int * ite_lamb, int * ite_cyc, int * gr_size_act, double *obj, double *runt, double *xinvc, double *lambda, int *nnlambda, double * ggamma, int *mmax_ite, double *pprec, double *LL, double *uinv, int *fflag){
    
    int i, j, k, m, p, n, d, max_ite1, max_ite2, nlambda, gr_size_a, gr_size_a1, ite1, ite2, c_idx, flag;
    double gamma, L, prec1, prec2, ilambda, tmp, dif1, dif2, neg1, pos1, np, np1, n1;
    clock_t start, stop;
    ...
    for (i=0; i<nlambda; i++) {
        ...
        while (dif1>prec1 && ite1<max_ite1) {
            ...
            prox_beta_est_mvr_l1(beta_tild, beta1, grad, S, L, ilambda/L, n1, p, d); // beta_tild = gr_soft(beta1-grad/L, ilambda)
            norm2_row_mat(grad_row2, beta_tild, d, p);
            identfy_actset(grad_row2, gr_act, &gr_size_a, d);
        ...
        ite_lamb[i] = ite1;
        obj[i] = get_obj_mvr(y_hat, beta1, xinvc, uinv, gr_act, gr_size_a, n, d, p,ilambda)*n1;
        stop = clock();
