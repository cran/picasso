...
void picasso_group_prox_orth(double *Y, double * X, double * beta, double * intcpt, int * nn, int * dd, int * ite_lamb, int * ite_cyc, int * gr_size_act, double *obj, double *runt, int * gr, int * gr_nn, int * gr_size, double *lambda, int *nnlambda, double * ggamma, int *mmax_ite, double *pprec, double *LL, int *fflag){
    
    int i, k, m, n, d, gr_n, max_ite1, max_ite2, nlambda, gr_size_a, gr_size_a1, ite1, ite2, c_idx, gr_idx, flag, idx;
    double gamma, L, prec1, prec2, ilambda, tmp, neg1, pos1, dif1, dif2, dbn, dbn1;
    clock_t start, stop;
    ...
    for (i=0; i<nlambda; i++) {
        ...
        while (dif1>prec1 && ite1<max_ite1) {
            ...
            prox_beta_est_gr(beta_tild, beta1, grad, L, ilambda/L, d, gr, gr_size, gr_n, dbn1); // beta_tild = soft(beta1-grad/L, ilambda)
