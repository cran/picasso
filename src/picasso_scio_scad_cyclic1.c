...
void picasso_scio_scad_cyclic1(double * S, double * beta, int * dd, int * ite1, double *lambda, int *nnlambda, int *mmax_ite, double *pprec, double * x, int *col_cnz, int *row_idx, double * ggamma){
    ...
    for (col=0; col<d; col++) {
        ...
        for (i=0; i<nlambda; i++) {
            ...
            for(j=0; j<d; j++){
                ...
                tmp = get_grad_scio_scad(e[j], S+j*d, beta1, set_act, size_a, ilambda, gamma, j);
