...
void picasso_scio_mcp_greedy1(double * S, double * beta, int * dd, int * ite_lamb, int * ite_cyc, double *lambda, int *nnlambda, int *mmax_ite, double *pprec, double * x, int *col_cnz, int *row_idx, double * ggamma){
    ...
    for (col=0; col<d; col++) {
        ...
        for (i=0; i<nlambda; i++) {
            ...
            while (dif1>prec1 && ite1<max_ite1) {
                ...
                get_grad_scio_mcp_vec(grad, e, S, beta1, set_act, size_a, ilambda, gamma, d);
                ...
                while (dif2>prec2 && ite2<max_ite2) {
                    for (m=0; m<size_a; m++) {
                        c_idx = set_act[m];
                        tmp = get_grad_scio_mcp(e[c_idx], S+c_idx*d, beta1, set_act, size_a, ilambda, gamma, c_idx);
